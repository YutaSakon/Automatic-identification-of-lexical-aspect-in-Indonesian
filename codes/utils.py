...
import os
import torch
from transformers import BertTokenizer, RobertaTokenizer, AlbertTokenizer, XLNetTokenizer, CamembertTokenizer, FlaubertTokenizer, XLMRobertaTokenizer
...
def tokenize_and_pad(sentences):
    ...
    if (args.transformer_model).split("-")[0] == 'xlm':
        tok = XLMRobertaTokenizer.from_pretrained(args.transformer_model)
    elif (args.transformer_model).split("-")[0] == 'bert':
    ...
    elif (args.transformer_model).split("-")[0] == 'flax':
        tok = RobertaTokenizer.from_pretrained(args.transformer_model)
    elif (args.transformer_model).split("-")[0] == 'cahya/bert':
        tok = BertTokenizer.from_pretrained(args.transformer_model)
    elif (args.transformer_model).split("-")[0] == 'indolem/indobert':
        tok = AutoTokenizer.from_pretrained(args.transformer_model)
